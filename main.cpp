...
// Custom deleter for 64-byte aligned memory on Windows
...
struct Matrix {
    ...
    Matrix(size_t rows, size_t cols)
        : rows(rows), cols(cols),
          data(static_cast<float*>(_aligned_malloc(rows * cols * sizeof(float), 64)), AlignedDeleter()) {
        if (!data) {
            throw std::bad_alloc();
        }
    }
...
void reset_matrix(Matrix& m) {
    std::fill_n(m.data.get(), m.rows * m.cols, 0.0f);
}
...
void single_threaded_packed_matrix_mult(const Matrix& a, const Matrix& b, Matrix& c) {
    ...
    constexpr size_t TILE_M = 96;
    constexpr size_t TILE_N = 96;
    constexpr size_t TILE_K = 96;
